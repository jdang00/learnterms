<div class="max-w-3xl px-10">
	<h1 class="mb-5 mt-5 text-4xl font-semibold">Blog</h1>
	<a
		href="https://learnterms.com/blog/LearnTerms%20v2"
		class="card lg:card-side bg-base-100 hover:bg-base-200"
	>
		<div class="card-body">
			<h2 class="card-title">LearnTerms v2 Release</h2>
			<p class="text-secondary text-sm">February 28, 2025</p>

			<p class="text-base-content">
				The future of our exam prep is here: LearnTerms v2 has officially arrived!
			</p>
		</div>
	</a>

	<a
		href="https://learnterms.com/blog/LearnTerms%20v2%20Beta"
		class="card lg:card-side bg-base-100 hover:bg-base-200"
	>
		<div class="card-body">
			<h2 class="card-title">LearnTerms v2 Beta</h2>
			<p class="text-secondary text-sm">Janurary 15, 2025</p>

			<p class="text-base-content">
				The next iteration of LearnTerms, custom built for general pharmacology.
			</p>
		</div>
	</a>

	<a
		href="https://learnterms.com/blog/LENS"
		class="card lg:card-side bg-base-100 hover:bg-base-200"
	>
		<div class="card-body">
			<h2 class="card-title">The LENS Algorithm: Playing the Numbers Game</h2>
			<p class="text-secondary text-sm">October 16, 2024</p>

			<p class="text-base-content">
				From definitions to domination: how LearnTerms turns studying into a numbers game you can
				win.
			</p>
		</div>
	</a>

	<a
		href="https://learnterms.com/blog/LearnTerms%201.0"
		class="card lg:card-side bg-base-100 hover:bg-base-200"
	>
		<div class="card-body">
			<h2 class="card-title">LearnTerms 1.0 Release</h2>
			<p class="text-secondary text-sm">September 23, 2024</p>

			<p class="text-base-content">
				What started as simple flashcards is now a quizzing powerhouse — LearnTerms 1.0 is live!
			</p>
		</div>
	</a>
</div>
