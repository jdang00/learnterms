<script lang="ts">
  ...
  function handleCardClick(event: MouseEvent, id: Id<'chunkContent'>) {
    ...
    if (target.closest('summary') || target.closest('input') || target.closest('button') || target.closest('a')) {
      return;
    }
    toggleSelect(id);
  }
...
{:else}
  ...
  <div class="grid gap-2 max-h-full overflow-y-auto overflow-x-hidden pr-1">
    {#each chunks.data as chunk (chunk._id)}
      <div class="card bg-base-100 border {isSelected(chunk._id) ? 'border-primary bg-primary/5' : 'border-base-300'} cursor-pointer transition-colors" role="button" tabindex="0" onclick={(e) => handleCardClick(e, chunk._id)} onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleCardClick(e as unknown as MouseEvent, chunk._id); } }} aria-pressed={isSelected(chunk._id)}>
        <div class="card-body p-3 gap-2">
          <div class="grid grid-cols-[1fr_auto] items-start gap-2 w-full">
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                ...
                <span class="badge badge-soft badge-primary badge-xs capitalize">{chunk.chunk_type}</span>
              </div>
              <div class="mt-1">
          ...
          <details class="collapse collapse-arrow bg-base-200">
            ...
            <div class="collapse-content">
              <pre class="whitespace-pre-wrap break-words text-xs leading-relaxed w-full max-w-full">{chunk.content}</pre>
            </div>
          </details>
        </div>
      </div>
