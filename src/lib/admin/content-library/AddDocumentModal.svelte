	...
	async function uploadDocument() {
		...
		try {
			...
			const documentId = await client.mutation(api.contentLib.insertR2Document, {
				...
				description: `${selectedFile.name} - ${formatBytes(selectedFile.size, 'Unknown size')}`,
				cohortId,
				metadata: {
			...
			await client.action(api.ragKnowledge.indexing.indexR2Document, {
				documentId
			});
	...
	<div class="modal-box w-full max-w-xl rounded-lg border border-base-300 shadow-2xl">
		...
		>
			...
			<span class="mt-1 text-xs text-base-content/60">
				{selectedFile ? formatBytes(selectedFile.size, 'Unknown size') : 'Maximum file size: 30MB'}
			</span>
