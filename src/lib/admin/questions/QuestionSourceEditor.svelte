<script lang="ts">
	...
	import QuestionStudioPagePicker from '../question-studio/QuestionStudioPagePicker.svelte';
	import type { SourcePreviewBatch } from '../source/sourceContext';
	import { createSourcePreviewCache } from '../source/sourcePreviewCache';
	import { createSourcePdf, warmPdfRenderer, type SourcePdfResource } from '../source/sourcePdf';
	...
	const previews = createSourcePreviewCache((id) =>
		client.action(api.questionStudio.context.getSourcePages, { documentId: id as Id<'contentLib'> })
	);
	...
	async function chooseDocument(id: Id<'contentLib'>, title: string) {
		...
		try {
			...
			const result =
				revision === undefined
					? await client.action(api.questionStudio.context.getSourcePages, { documentId: id })
