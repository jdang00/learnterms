<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			await client.mutation(api.contentLib.insertDocument, {
			...
			submitError = error instanceof Error ? error.message : 'Failed to create document';
			console.error('Failed to create document:', submitError);
		} finally {
			isSubmitting = false;
		}
	}
		...
		<div class="modal-action mt-8">
			<form method="dialog" class="flex gap-3">
				...
				<button
					...
					title={!isFormValid
						? 'Please fill all fields correctly before creating the document'
						: ''}
				>
					{#if isSubmitting}
