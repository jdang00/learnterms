...
<dialog class="modal max-w-full p-4" class:modal-open={isOpen}>
  <div class="modal-box">
    ...
    <div class="mt-4 space-y-4">
      ...
      <p class="text-xs text-base-content/70">Automatically advance to the next question after a correct answer.</p>
      ...
      <p class="text-xs text-base-content/70">Randomize the order of answer choices across all questions.</p>

      <div class="flex justify-between items-center">
        <span>Reset progress</span>
        <button class="btn btn-error btn-soft btn-sm" onclick={() => { isOpen = false; qs.isResetModalOpen = true; }}>
          <RotateCcw size={16} />
          <span class="ml-1 hidden sm:inline">Reset</span>
        </button>
      </div>
      <p class="text-xs text-base-content/70">Clears selections, eliminations, flags, and saved progress for this chapter.</p>
