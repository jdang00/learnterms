<script lang="ts">
	...
	function shouldSuppressError(error: unknown, hasOtherAuthErrors: boolean = false): boolean {
		if (!isConvexAuthError(error)) return false;

		const errorMessage = getErrorText(error).toLowerCase();
...
{#if !shouldSuppress}
	<div
		class="rounded-lg border-l-4 {isAuthError
			? 'border-primary bg-primary/5'
			: 'border-error bg-error/5'} p-4 shadow-xs {className}"
		role="alert"
	>
				...
				{#if isAuthError}
					...
					<div class="text-sm text-base-content/80">
						...
						<p class="text-base-content/60">
							This automatic refresh helps protect your learning data.
						</p>
					</div>
				{:else}
