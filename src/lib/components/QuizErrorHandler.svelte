<script lang="ts">
	...
	let { questions, module, moduleProgress, className = '' } = $props();
	...
	function getErrorPriority(error: unknown): number {
		...
		if (getErrorText(error).toLowerCase().includes('network')) return 2;
		return 1;
	}
		...
		[
			{
				...
				source: 'questions',
				priority: getErrorPriority(questions?.error)
			},
			{ error: module?.error, source: 'module', priority: getErrorPriority(module?.error) },
			{
				error: moduleProgress?.error,
				source: 'progress',
				priority: getErrorPriority(moduleProgress?.error)
			}
	...
	let primaryError = $derived(
		allErrors.length > 0
			? allErrors.reduce((prev, current) => (current.priority > prev.priority ? current : prev))
			: null
	);
	...
	function getConsolidatedError(): { title: string; message: string } {
		...
		if (hasMultipleAuthErrors) {
			return {
				...
				message:
					'Your session has been refreshed for security. Multiple services need to reconnect. Please reload to continue learning.'
			};
		} else if (hasAuthErrors && hasNonAuthErrors) {
			return {
				title: 'Authentication Required',
				message:
					'Your session needs to be refreshed and some services are temporarily unavailable. Please reload to restore full functionality.'
			};
