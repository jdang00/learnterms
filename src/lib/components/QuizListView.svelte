...
{:else}
	<div class="space-y-4">
		{#each questions as questionItem, index (questionItem._id)}
			<div
				class="group relative rounded-2xl border border-base-200 bg-base-100 p-5 transition-all hover:border-base-300 hover:shadow-xs"
			>
						...
						<div class="flex-1 min-w-0">
							<div class="flex items-start justify-between gap-4">
								...
								{#if questionItem.type}
									<span
										class="badge badge-xs badge-outline text-[10px] uppercase tracking-wider opacity-50 flex-none mt-1"
									>
					...
					{#if questionItem.options?.length}
						<div class="grid grid-cols-1 gap-2 pl-2 sm:pl-10">
							{#each questionItem.options as option, i (option.id)}
								<div
									class="flex items-start gap-3 rounded-xl bg-base-200/50 px-4 py-3 text-sm transition-colors hover:bg-base-200"
								>
									...
									<div class="flex-1 min-w-0 tiptap-content leading-snug">
										<span
											class={questionItem.correctAnswers?.includes(option.id)
