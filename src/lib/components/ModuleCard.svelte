	...
	>
		...
		<div class="relative p-5 flex flex-col h-full">
			<div class="flex items-start gap-3 mb-3">
				<span
					class="text-3xl leading-none shrink-0 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3"
					>{module.emoji || '📘'}</span
				>
				<div class="min-w-0 flex-1">
					<h2
						class="font-semibold text-base-content text-left truncate text-[0.95rem] leading-snug mb-1.5"
					>
						{module.title}
					</h2>
					<div class="flex items-center gap-2 flex-wrap">
						{#if module.questionCount !== undefined}
							<span
								class="inline-flex items-center gap-1 text-xs text-base-content/50 bg-base-200/60 rounded-full px-2 py-0.5"
							>
								<BookOpen size={11} />
								...
								>
									<div class="flex flex-col gap-0.5">
										{#each module.tags as tag (tag._id)}
											<div
												class="flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-xs hover:bg-base-200/70 transition-colors duration-150"
											>
												<span
			...
			<div class="mt-auto flex items-center justify-between">
				<div
					class="btn btn-sm btn-primary btn-soft rounded-full gap-1 transition-all duration-200 group-hover:gap-2 ml-auto"
				>
					Start <ArrowRight
						size={14}
...
<div in:fade={{ duration: 300 }} class="relative">
	...
	{:else}
		<a href={resolve('/classes/[classId]/modules/[moduleId]', { classId, moduleId: module._id })}>
			{@render cardContent()}
		</a>
	{/if}
</div>
