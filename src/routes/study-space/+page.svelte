<script lang="ts">
	...
	const mockSources = [
		{ id: 0, name: 'Ch. 12 — Cardiac Physiology', type: 'pdf', pages: 28, icon: '❤️', added: '2 days ago' },
		{ id: 1, name: 'Lecture 14 Notes', type: 'notes', pages: 6, icon: '📝', added: '1 day ago' },
		{ id: 2, name: 'Pharmacology Review Sheet', type: 'pdf', pages: 12, icon: '💊', added: '3 days ago' },
		{ id: 3, name: 'Renal System Overview', type: 'slides', pages: 45, icon: '🫘', added: '5 days ago' },
		{ id: 4, name: 'Pathology Case Studies', type: 'pdf', pages: 18, icon: '🔬', added: '1 week ago' },
	];

	const mockChat = [
		{
			role: 'assistant',
			content: "Welcome to your Study Space! I've loaded 5 sources about cardiology and pharmacology. What would you like to explore first?",
			timestamp: '2:34 PM'
		},
		{
			role: 'user',
	...
	const mockQuestions: MockQuestion[] = [
		{
			...
			correctAnswers: ['b'],
			rationale: 'The Frank-Starling law states that the stroke volume of the heart increases in response to an increase in the volume of blood filling the heart (end-diastolic volume). The increased stretch of the cardiac muscle fibers leads to a more forceful contraction.',
		...
		},
		{
			...
			correctAnswers: ['a'],
			rationale: 'Phase 0 of the cardiac action potential involves the rapid opening of voltage-gated Na⁺ channels, causing a swift depolarization from approximately -90mV to +20mV. This is the fastest phase and is responsible for the upstroke of the action potential.',
		...
		},
		{
			...
			correctAnswers: ['c'],
			rationale: 'Baroreceptor-mediated sympathetic activation is the most immediate compensatory response to decreased cardiac output. Baroreceptors in the carotid sinus and aortic arch detect the drop in pressure and trigger increased sympathetic tone within seconds, increasing heart rate and contractility.',
		...
		},
		{
			...
			correctAnswers: ['c'],
			rationale: 'During isovolumetric contraction, both the AV valves (mitral and tricuspid) and semilunar valves (aortic and pulmonic) are closed. Ventricular pressure is rising but has not yet exceeded aortic pressure, so no blood is ejected and volume remains constant.',
	...
	const interactedCount = $derived(mockQuestions.filter(q => q.interacted).length);
	const progressPercent = $derived(Math.round((interactedCount / mockQuestions.length) * 100));
	...
	function checkAnswer() {
		...
		const correct = currentQuestion.correctAnswers.every(a => selectedOptions.includes(a))
	...
	function goToQuestion(index: number) {
		...
		checkResult = '';
		showRationale = false;
		isFlagged = mockQuestions[index].flagged;
	}
	...
	function getOptionState(optionId: string): 'default' | 'selected' | 'eliminated' | 'correct' | 'incorrect' {
		if (checkResult) {
	...
	<div class="relative z-10 flex flex-col h-[calc(100vh-4rem)] overflow-hidden">
		...
		>
			<div class="flex items-center gap-3">
				<a href={resolve('/classes')} class="btn btn-sm btn-ghost rounded-full hover:bg-base-200/80">
					<ArrowLeft size={16} />
				</a>
				<div class="h-5 w-px bg-base-300"></div>
				<div class="flex items-center gap-2.5">
					<div class="w-8 h-8 rounded-xl bg-gradient-to-br from-primary/20 to-secondary/20 flex items-center justify-center">
						<span class="text-sm">❤️</span>
					</div>
					<div>
						<h1 class="text-sm font-semibold text-base-content leading-tight">Cardiovascular System</h1>
						<p class="text-[11px] text-base-content/45">BIO 301 · 5 sources · Last studied 2h ago</p>
					</div>
				</div>
			</div>
			...
			<div class="flex items-center gap-1.5">
				...
				<button class="btn btn-sm btn-ghost btn-circle text-base-content/50">
					<Settings size={15} />
				</button>
			</div>
		</div>
		...
		<div class="flex flex-1 overflow-hidden">
			...
			{#if leftPanelOpen}
				...
				>
					...
					{#if activeTab === 'sources'}
						<!-- Add sources area -->
						<div class="p-3">
							<button class="group w-full flex items-center gap-3 rounded-xl border-2 border-dashed border-base-300 hover:border-primary/40 p-3 transition-all duration-200 hover:bg-primary/5">
								<div class="w-9 h-9 rounded-lg bg-base-200 group-hover:bg-primary/15 flex items-center justify-center transition-all duration-200">
									<Plus size={16} class="text-base-content/40 group-hover:text-primary transition-colors" />
								</div>
								<div class="text-left">
									<div class="text-xs font-medium text-base-content/70 group-hover:text-base-content">Add source</div>
									<div class="text-[10px] text-base-content/40">PDF, notes, slides, or paste text</div>
								</div>
							</button>
						</div>
						...
						<div class="flex-1 overflow-y-auto px-3 pb-3 space-y-1">
							{#each mockSources as source, i (source.id)}
								<button
									...
									style="animation: sourceReveal 0.35s cubic-bezier(.16,1,.3,1) {i * 60 + 100}ms both;"
								>
									<div class="flex items-start gap-2.5">
										<span class="text-lg leading-none mt-0.5 shrink-0">{source.icon}</span>
										<div class="flex-1 min-w-0">
											<div class="text-xs font-medium text-base-content truncate leading-tight">{source.name}</div>
											<div class="flex items-center gap-1.5 mt-1">
												<span class="badge badge-xs badge-ghost rounded-full text-[10px] uppercase tracking-wider">{source.type}</span>
						...
						<div class="p-3 border-t border-base-300/50">
							<div class="flex items-center justify-between">
								<span class="text-[11px] text-base-content/40">{mockSources.length} sources loaded</span>
								<button class="text-[11px] text-primary hover:text-primary/80 font-medium transition-colors">
									Manage
								</button>
							</div>
						</div>
					{:else}
						...
						<div class="flex-1 p-3">
							<div class="rounded-xl bg-base-200/50 border border-base-300/50 p-4 h-full">
								...
								<div class="text-xs text-base-content/40 leading-relaxed" contenteditable="true" role="textbox">
			...
			<div class="flex-1 flex flex-col overflow-hidden bg-gradient-to-b from-base-100/40 to-base-200/30 backdrop-blur-xs">
				<!-- Chat messages -->
				<div class="flex-1 overflow-y-auto px-4 sm:px-8 lg:px-16 py-6">
					<div class="max-w-2xl mx-auto space-y-6">
						{#each mockChat as message, i (i)}
							...
							>
								{#if message.role === 'assistant'}
									<div class="w-8 h-8 rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center shrink-0 mt-1">
								...
								<div class="flex-1 max-w-[85%] {message.role === 'user' ? 'ml-auto' : ''}">
									<div class="rounded-2xl px-4 py-3 {message.role === 'user'
										...
										<div class="text-sm leading-relaxed whitespace-pre-line {message.role === 'assistant' ? 'chat-content' : ''}">
										...
										{#if message.role === 'assistant' && message.sources}
											<div class="mt-3 pt-2.5 border-t border-base-300/40">
												<div class="flex items-center gap-1.5 flex-wrap">
													<span class="text-[10px] text-base-content/35 uppercase tracking-wider font-medium">Sources:</span>
													{#each message.sources as src, i (`${src}-${i}`)}
														<span class="badge badge-xs badge-ghost rounded-full text-[10px]">{src}</span>
													{/each}
												</div>
											</div>
										{/if}
						...
						>
							<span class="text-[10px] text-base-content/35 uppercase tracking-wider font-medium px-1">Suggested</span>
							<div class="flex flex-wrap gap-2">
								{#each ['Explain pressure-volume loops', 'Quiz me on cardiac drugs', 'Compare systolic vs diastolic failure', 'Summarize Ch. 12 key points'] as suggestion (suggestion)}
									<button class="group flex items-center gap-1.5 rounded-full border border-base-300/60 bg-base-100/80 px-3.5 py-1.5 text-xs text-base-content/60 hover:border-primary/40 hover:text-primary hover:bg-primary/5 transition-all duration-200">
										<Zap size={11} class="text-base-content/30 group-hover:text-primary transition-colors" />
										{suggestion}
									</button>
								{/each}
							</div>
				...
				<!-- Input area -->
				<div class="border-t border-base-300/40 bg-base-100/70 backdrop-blur-xl px-4 sm:px-8 lg:px-16 py-4">
					<div class="max-w-2xl mx-auto">
						<div class="flex items-end gap-2 rounded-2xl bg-base-100 border border-base-300/60 shadow-xs p-2 focus-within:border-primary/40 focus-within:shadow-md transition-all duration-200">
							<button class="btn btn-sm btn-ghost btn-circle text-base-content/40 shrink-0 self-end">
								<Plus size={16} />
							</button>
							...
							<div class="flex items-center gap-1 shrink-0 self-end">
								...
								<button class="btn btn-sm btn-primary rounded-full px-3 gap-1 {chatInput.trim() ? '' : 'btn-disabled opacity-40'}">
									<Send size={13} />
								</button>
							</div>
						</div>
						<p class="text-[10px] text-base-content/30 text-center mt-2">
							AI responses are generated from your uploaded sources. Always verify important information.
						</p>
					</div>
				</div>
			</div>
				...
				>
					...
					<div class="flex items-center justify-between px-3.5 py-2.5 border-b border-base-300/50">
						<div class="flex items-center gap-2">
							...
							<span class="badge badge-xs badge-primary badge-soft rounded-full">{mockQuestions.length} Qs</span>
						</div>
						<div class="flex items-center gap-1">
							<button class="btn btn-xs btn-ghost btn-circle text-base-content/40" title="Shuffle">
								<Shuffle size={12} />
							</button>
							<button class="btn btn-xs btn-ghost btn-circle text-base-content/40" title="Reset">
								<RotateCcw size={12} />
							</button>
						</div>
					</div>
					...
					<div class="px-3.5 py-2 border-b border-base-300/30">
						<div class="flex items-center gap-1">
							{#each mockQuestions as q, i (q.id)}
								<button
								...
								>
									{i + 1}
									{#if q.flagged}
										<div class="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-warning border border-base-100"></div>
									{/if}
								</button>
							{/each}
						</div>
						<div class="flex items-center justify-between mt-1.5">
							<span class="text-[10px] text-base-content/35">{interactedCount}/{mockQuestions.length} touched</span>
							<div class="flex items-center gap-1.5">
						...
						{#key currentQuestionIndex}
							<div style="animation: questionSlideIn 0.3s cubic-bezier(.16,1,.3,1) both;">
								...
								<div class="flex items-center gap-1.5 mb-3">
									<span class="badge badge-xs badge-ghost rounded-full text-[9px] uppercase tracking-widest">
								...
								<div class="space-y-1.5">
									{#each currentQuestion.options as option, i (option.id)}
										...
										<div
											...
											style="animation: optionReveal 0.25s cubic-bezier(.16,1,.3,1) {i * 50 + 80}ms both;"
										>
											<button
												...
												oncontextmenu={(e) => { e.preventDefault(); toggleElimination(option.id); }}
											>
												<div class="flex items-start gap-2.5">
													<span class="shrink-0 w-5 h-5 rounded-md flex items-center justify-center text-[10px] font-bold mt-px
														{state === 'selected'
								...
								{#if showRationale || checkResult === 'correct'}
									...
									>
										<div class="flex items-center gap-1.5 mb-1.5">
											...
											<span class="text-[10px] font-semibold text-base-content/50 uppercase tracking-wider">Rationale</span>
										</div>
					...
					<div class="px-3.5 py-2.5 border-t border-base-300/40 bg-base-100/80 backdrop-blur-xs">
						<div class="flex items-center gap-1.5">
							...
							<div class="flex items-center gap-1">
								<button
									class="btn btn-xs btn-ghost btn-circle text-base-content/40 {currentQuestionIndex === 0 ? 'btn-disabled opacity-30' : ''}"
								...
								<button
									class="btn btn-xs btn-ghost btn-circle text-base-content/40 {currentQuestionIndex === mockQuestions.length - 1 ? 'btn-disabled opacity-30' : ''}"
									onclick={nextQuestion}
								>
