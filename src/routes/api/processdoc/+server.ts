  ...
  return {
    ...
    items: {
      ...
      properties: {
        ...
        keywords: { type: (Type as unknown as { ARRAY: unknown }).ARRAY, items: { type: (Type as unknown as { STRING: unknown }).STRING } },
        chunk_type: {
...
Instructions:
1) Create 5-15 substantial chunks
2) ${tableGuidelines()}
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (file.type !== 'application/pdf') return json({ error: 'File must be a PDF' }, { status: 400 });
    ...
    if (contentType.includes('application/json')) {
      const body = (await request.json()) as { material?: string; pdfUrl?: string; fileKey?: string };
