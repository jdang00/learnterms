		...
		<form>
			{#if answerstatus === AnswerStatus.correct}
				...
				<input
					...
					class="input input-bordered input-success w-full max-w-xs mt-5"
					bind:this={ref}
					bind:value={input}
				/>
			...
			{:else}
				...
				/>
			{/if}
			<button type="submit" class="btn btn-primary mt-3 ms-3" on:click={checkAnswer}>Enter</button>
		</form>
				...
				<div class="card-body">
					{#if showAnswer === false}
						<div class="flex justify-between">
							...
							<button class="btn" on:click={() => (showAnswer = true)}><Eye /></button>
						</div>
					{:else}
						<div class="flex justify-between">
							<h2 class="card-title">{answer}</h2>
							<button class="btn" on:click={() => (showAnswer = false)}><Eye /></button>
						</div>
					{/if}
				</div>
			</div>

			<button class="btn" on:click={() => nextFlashcard()}><ChevronRight /></button>
		</div>
	</div>
</div>
