...
export class LogicMap {
	questions: AdminQuestions[] = $state([]);
	...
	editingQuestion = $state<AdminQuestions | null>(null);
	isSaving = $state(false);
	saveError = $state<string | null>(null);

	// Add question modal state
	isAddModalOpen = $state(false);
	newQuestion = $state<AdminQuestions | null>(null);
	...
	openEditModal(question: AdminQuestions) {
	...
	async handleDelete() {
		...
		const { error } = await supabase.from('pharmquestions').delete().eq('id', this.pendingDeleteId);
