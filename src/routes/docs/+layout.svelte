<script lang="ts">
  ...
  const prevItem: DocsNavItem | null = $derived(currentIndex > 0 ? flatItems[currentIndex - 1] : null)
  ...
  const pageTitle = $derived(
    (flatItems[currentIndex]?.title ? `${flatItems[currentIndex]?.title} – ` : '') + 'LearnTerms Docs'
  )
