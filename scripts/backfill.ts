...
async function runBatch(cursor: string | null) {
	...
	const result = spawnSync(
		'bun',
		['convex', 'run', 'migrations:backfillFlagCounts', args, DEPLOYMENT_FLAG].filter(Boolean),
		{ encoding: 'utf-8' }
	);
...
async function main() {
	...
	while (!done) {
		...
		console.log(
			`Progress: Processed ${totalProcessed} questions, Updated ${totalUpdated} flagCounts.`
		);
